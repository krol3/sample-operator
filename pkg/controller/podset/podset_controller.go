...
import (
	...
	appv1alpha1 "github.com/krol/sample-operator/pkg/apis/app/v1alpha1"
...
func (r *ReconcilePodSet) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	reqLogger.Info("Checking podset", "expected replicas", instance.Spec.Replicas, "Pod.Names", existingPodNames)
	...
	if int32(len(existingPodNames)) > instance.Spec.Replicas {
		...
		reqLogger.Info("Deleting a pod in the podset", "expected replicas", instance.Spec.Replicas, "Pod.Names", existingPodNames)
	...
	if int32(len(existingPodNames)) < instance.Spec.Replicas {
		...
		reqLogger.Info("Adding a pod in the podset", "expected replicas", instance.Spec.Replicas, "Pod.Names", existingPodNames)
